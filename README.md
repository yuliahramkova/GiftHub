...
- База данных только в оперативной памяти — после перезапуска `backend` новые пользователи и изменения теряются (демо-аккаунты восстанавливаются при старте).
- API URL захардкожен как `http://localhost:3001` во фронтенде.
